...
def get_build_keys(product_config):
  ...
  if default_cert == "" or default_cert == os.path.join(TEST_KEY_DIR, "testKey"):
    return "test-keys"
  if default_cert.startswith("vendor/xperience/signing/keys/"):
    return "release-keys"
  return "dev-keys"
...
def parse_args():
  ...
  config["XPerienceDesc"] = config["BuildDesc"]
  config["XPerienceDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] == "user":
    ...
    print(f"ro.build.display.id?={config['XPerienceDesc']}")
  ...
  print(f"ro.xpe.device={config['XPerienceDevice']}")
